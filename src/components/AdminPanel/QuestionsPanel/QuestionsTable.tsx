...
function Row(props: { row: Question }) {
  const { row } = props;
  const { setFormData, handleOpen, isGameView, deleteQuestion  } = React.useContext(QuestionsContext);
        ...
        <TableCell style={{ paddingBottom: 0, paddingTop: 0 }} colSpan={6}>
          <Collapse in={open} timeout="auto" unmountOnExit>
            <QuestionDetail row={row}>
              ...
              <IconButton sx={{color: '#f04462'}} onClick={()=>{deleteQuestion(row.questionId)}}> <DeleteOutlineOutlinedIcon /></IconButton>
              </div>
            </QuestionDetail>
          </Collapse>
        </TableCell>
